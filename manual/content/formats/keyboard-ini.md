...
A command that carries a forced binding is bound again once the file has been processed, taking that key back from whatever the file gave it. The file's own binding for that command is left alone, so it can end up answering to two keys. Delete, Escape, V, T, and Y are forced this way: Delete deletes a waypoint, Escape opens options, V toggles remastered graphics, T toggles remastered textures, and Y toggles remastered mesh density.
