...
players, and optional worst-local RTT after 32 and 64 frames, then every 128
frames. A player omits the RTT while any of its links has no
[clean measurement](/systems/network-transport-timing/); a link keeps reporting
its last measurement while it retransmits. The deterministic master evaluates
at 64 and 128 frames, then every 256 frames.
...
scheduled frame unchanged. A later target re-stages from the timing in force.
...
look-ahead is also Bad. The message list announces target-tier changes, which
may precede a safely staged improvement. The Speed slider controls game speed.
...
snapshot that wrote them. Existing event IDs retain their values. The policy
reads measured RTT directly; `LATENCYFUDGE` keeps its event ID and session
field for replay compatibility, and nothing emits or consumes it.
